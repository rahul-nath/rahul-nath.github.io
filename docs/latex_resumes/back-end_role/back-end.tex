\documentclass[]{deedy-resume-openfont}

% \hypersetup{%
%   colorlinks=true,% hyperlinks will be black
%   urlcolor=cyan
% }

\begin{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     TITLE NAME
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\namesection{Rahul}{Nath}{ \urlstyle{same} \underline{\url{http://therahulnath.com}} |  \underline{\href{mailto:rahul.nath.eph@gmail.com}{rahul.nath.eph@gmail.com}}\\ 516.491.9232 | Github: \underline{\href{https://github.com/rahul-nath}{rahul-nath}}
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     COLUMN ONE
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{minipage}[t]{0.31\textwidth} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EDUCATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Education} 
\\[1\baselineskip]
\subsection{\underline{\href{https://www.timeshighereducation.com/world-university-rankings/2017/subject-ranking/computer-science\#!/page/0/length/25/sort_by/rank_label/sort_order/asc/cols/rank_only}{Georgia Tech}}}
\descript{M.S. in Computer Science}
\location{Part-Time | \underline{\href{http://www.nytimes.com/2016/09/29/upshot/an-online-education-breakthrough-a-masters-degree-for-a-mere-7000.html}{OMSCS}}}
{\footnotesize \textit{\textbf{Specialization in Machine Learning}}} 
\sectionsep

\subsection{Udacity}
\descript{N.D. in Full-Stack WebDev}
\location{Expected December 2016 | Udacity}
\sectionsep

\subsection{\underline{\href{http://www.forbes.com/top-colleges/list/}{Williams College}}}
\descript{B.S. in Computer Science \\ B.A. in Economics}
\location{June 2015 | Williamstown, MA}
{\footnotesize \textit{\textbf{Dean's List}}}, {\footnotesize \textit{\textbf{Class of 1960 Economics Scholar}}}
\sectionsep

\subsection{GCHS, Valedictorian}
\location{June 2011 |  Glen Cove, New York}
\sectionsep

\subsection{Relevant Coursework}
High Performance Parallel Algorithms
Data Structures \& Algorithms
{\footnotesize \textit{\textbf{(Also T.A.'d) }}} \\
Operating Systems
\\[1\baselineskip]

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     SKILLS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Skills}
\subsection{Programming}

\location{Main Languages:}
Python \textbullet{}  SQL  \textbullet{} C \textbullet{} Java 
\\[1\baselineskip]

\location{Familiarity with:}
Javascript \textbullet{} Scala \textbullet{}  C++  \textbullet{} Lisp
\\[1\baselineskip]

\location{Recent Experience with:}
HTML/CSS \textbullet{} Google App Engine \\
\textbullet{}  Unix/Bash \textbullet{} Docker \textbullet{}  Vagrant \textbullet{} AWS EC2/Redshift \textbullet{} PostgreSQL \textbullet{}  Chef
\\[1\baselineskip]

\location{Past Experience with:}
Selenium \textbullet{} Ruby on Rails \\
\textbullet{}  Node.js \textbullet{}  Heroku \textbullet{}  BeautifulSoup
\sectionsep

\location{Third-Party Tools:}
Chartio \textbullet{}  Optimizely A/B \textbullet{}  Slack \\
\textbullet{}  Git \textbullet{}  JIRA \textbullet{} Trello \textbullet{} Asana \textbullet{} Reflektiv
\sectionsep

\subsection{Spoken Languages}
Spanish (Intermediate Proficiency) \\
French (Beginner Proficiency)\\
Bengali (Intermediate Proficiency)
\\[1\baselineskip]

\section{Open-Source}
\textbullet{} String matching: \underline{\href{https://github.com/seatgeek/fuzzywuzzy}{Fuzzy-Wuzzy}} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     COLUMN TWO
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\end{minipage} 
\hfill
\begin{minipage}[t]{0.66\textwidth} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EXPERIENCE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Experience}

\runsubsection{Udacity}
\descript{| Course Manager/Software Developer}
\location{Aug 2015 – July 2016 | Mountain View, CA}
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightemize}

\item Re-invented Intro to Programming Nanodegree (IPND) into exploratory program to other NDs, increasing student enrollment by over 20\% -- the fastest growing ND at Udacity.

\item Created webcast teaching sessions and authored lessons on virtualization and Python optimization constructs.

\item QA'd and created content for Machine Learning \& Android Basics.

\end{tightemize}
\sectionsep

\runsubsection{Naval Research Laboratory}
\descript{| Contractor }
\location{July 2014 – Sept 2014 | National Harbor, MD}
\begin{tightemize}
\item Implemented a tool in Python for A.I. robot to learn actions in polynomial time -- a previously intractable problem -- making it feasible to automate the process of knowledge acquisition.

\item Designed and implemented an algorithm in Lisp and Python to reduce prepositional and word-sense ambiguity in interpreted speech using contextual information and machine learning.
\end{tightemize}
\sectionsep

\runsubsection{Mobiquity}
\descript{| Android Developer Intern}
\location{May 2014 – July 2014 | Wellesley, MA}
\begin{tightemize}
\item Implemented communication and data collection between an Amazon EC2 server instance and an Android application for Mobiquity’s deltaIQ platform, which forms the basis of their health monitoring service.

\item Created the UX, UI, and business logic for an indoor navigation app using Node.js, iBeacons, Google Glass, and AWS EC2, DynamoDB, S3, Kinesis, and Cognito.
\end{tightemize}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     Projects
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Projects}

\location{TA Sessions}
\begin{tightemize}
\item I conceptualized and developed Udacity's first student-to-student video platform with integrated Slack orientation bot for over 2000 students. Leveraged Google Cloud Platform and Ruby on Rails.
\end{tightemize}
\sectionsep

\location{YouTube Data Pull}
\begin{tightemize}
\item Automated student YouTube data collection for all of Udacity. I wrote a Python cron job that queried YouTube Analytics servers collecting over 8,000,000 data points. I then formatted and stored the data on Udacity's AWS RedShift server, and pipelined it into Chartio as a data source for analysis.
\end{tightemize}
\sectionsep

\location{\underline{\href{https://github.com/rahul-nath/PrePost2/blob/master/final_try.py}{PrePost2}}}
\begin{tightemize}
\item Using selenium, multiprocessing pools, and BeautifulSoup, I created a webscraper to collect documents and suggest 25\% more accurate environment domain models to be used by an artificially intelligent agent. 
\end{tightemize}
\sectionsep

\location{\underline{\href{https://github.com/rahul-nath/medicost}{Medicost}}    (MIT Hack Medicine Team, 24-hr hackathon)}
\begin{tightemize}
\item Created website to easily search cost requests and receipts made by doctors, standardizing medical procedure costs for over 44 million Americans without medical insurance. This project used Django, Grunt, JQuery, and Bootstrap.
\end{tightemize}
\sectionsep

\location{\underline{\href{https://github.com/rahul-nath/SchoolStuff/blob/master/classes/comp_sci/cs237/wave2/wave3.s}{Virtual Assembly Language Emulation}}}
\begin{tightemize}
\item I wrote a virtual emulator for a RISC language using a CISC language. I made a JIT compiler to optimize, finding a trade-off of 26 CISC instructions for every RISC instruction.
\end{tightemize}


\end{minipage} 
\end{document}  \documentclass[]{article}
